...
def timeticks_to_str(ticks):
    ...
    return "{}{:-02}:{:-02}:{:-02}.{:-02}".format(
        days_fmt, hours, minutes, seconds, milliseconds
    )
...
def _parse_asn1_opaque(stream):
    ...
    if (
        length == ASN1_OPAQUE_FLOAT_BER_LEN
        and opaque_tag == ASN1_OPAQUE_TAG1
        and opaque_type == ASN1_OPAQUE_FLOAT
    ):
        return _parse_asn1_opaque_float(stream)
    elif (
        length == ASN1_OPAQUE_DOUBLE_BER_LEN
        and opaque_tag == ASN1_OPAQUE_TAG1
        and opaque_type == ASN1_OPAQUE_DOUBLE
    ):
        return _parse_asn1_opaque_double(stream)
    elif (
        length >= ASN1_OPAQUE_INT64_BER_LEN
        and opaque_tag == ASN1_OPAQUE_TAG1
        and opaque_type == ASN1_OPAQUE_INT64
    ):
        return _parse_asn1_opaque_int64(stream)
    elif (
        length >= ASN1_OPAQUE_UINT64_BER_LEN
        and opaque_tag == ASN1_OPAQUE_TAG1
        and opaque_type == ASN1_OPAQUE_UINT64
    ):
...
def _validate_protocol(pdu_index, tag, result):
    ...
    if _is_trap_request(result):
        if (
            pdu_index == 4
            and tag != ASN1_OBJECT_IDENTIFIER
            or pdu_index == 5
            and tag != ASN1_IPADDRESS
            or pdu_index in [6, 7]
            and tag != ASN1_INTEGER
            or pdu_index == 8
            and tag != ASN1_TIMETICKS
        ):
            return False
    elif (
        pdu_index in [1, 4, 5, 6]
        and tag != ASN1_INTEGER
        or pdu_index == 2
        and tag != ASN1_OCTET_STRING
        or pdu_index == 3
        and tag
        not in [
...
def _parse_snmp_asn1(stream):
    ...
    while True:
        ...
        if tag == ASN1_SEQUENCE:
            ...
            length = _parse_asn1_length(stream)
            logger.debug(
                "ASN1_SNMPv2_TRAP_REQUEST_PDU: %s", "length = {}".format(length)
            )
            if pdu_index == 3:  # PDU-type
...
def bit_string(value):
    ...
    """
    return write_tlv(
        ASN1_BIT_STRING, len(value), value.encode("latin") if PY3 else value
    )
...
def real(value):
    ...
    opaque_type_value = (
        struct.pack("BB", ASN1_OPAQUE_TAG1, ASN1_OPAQUE_FLOAT)
        + _write_asn1_length(len(float_value))
        + float_value
    )
...
def double(value):
    ...
    opaque_type_value = (
        struct.pack("BB", ASN1_OPAQUE_TAG1, ASN1_OPAQUE_DOUBLE)
        + _write_asn1_length(len(double_value))
        + double_value
    )
    return write_tv(ASN1_OPAQUE, opaque_type_value)
...
def int64(value):
    ...
    opaque_type_value = (
        struct.pack("BB", ASN1_OPAQUE_TAG1, ASN1_OPAQUE_INT64)
        + _write_asn1_length(len(int64_value))
        + int64_value
    )
    return write_tv(ASN1_OPAQUE, opaque_type_value)
...
def uint64(value):
    ...
    opaque_type_value = (
        struct.pack("BB", ASN1_OPAQUE_TAG1, ASN1_OPAQUE_UINT64)
        + _write_asn1_length(len(uint64_value))
        + uint64_value
    )
    return write_tv(ASN1_OPAQUE, opaque_type_value)
        ...
        with open(filename, "rb") as conf_file:
            ...
            for value in oids.values():
                if (
                    ...
                    and value.__code__.co_argcount != 1
                ):
                    raise ConfigError(
...
def craft_response(
    ...
    response = write_tv(
        ...
        + write_tv(
            ASN1_OCTET_STRING, community.encode("latin") if PY3 else str(community)
        )
        +
...
def check_config_updates(oids):
    """Monitor config file for changes and reload when modified"""
        ...
        try:
            if config_file and os.path.exists(config_file):
                ...
                if current_mtime > config_last_modified:
                    ...
                    try:
                        new_oids = parse_config(config_file)
                        with oids_lock:
                            oids.clear()
                            oids.update(new_oids)
                        config_last_modified = current_mtime
                        logger.info("Config reloaded successfully")
                    except Exception as ex:
                        logger.error("Failed to reload config: %s", ex)
        except Exception as ex:
            logger.error("Error checking config updates: %s", ex)
        time.sleep(1)
    ...
    with closing(
        ...
        while True:
            try:
                ...
                # Use the lock when accessing oids
                with oids_lock:
                    ...
                    if len(request_result) < expected_length:
                        raise Exception(
                            "Invalid ASN.1 parsed request length! %s"
                            % str(request_result)
                        )
                    ...
                    if pdu_type == ASN1_GET_REQUEST_PDU:
                        ...
                        error_status, error_index, oid, oid_value = (
                            handle_get_next_request(oids, oid)
                        )
                        if isinstance(oid_value, types.FunctionType):
                            oid_value = oid_value(oid)
                        if isinstance(oid_value, tuple):
                        ...
                        try:
                            if (
                                oid in oids
                                and isinstance(oids[oid], tuple)
                                and len(oids[oid]) > 1
                            ):
                                enum_values = oids[oid][1]
                                new_value = type_and_value[1]
                                if (
                                    isinstance(enum_values, Iterable)
                                    and new_value not in enum_values
                                ):
                                    raise WrongValueError(
                                        "Value {} is outside the range of enum values".format(
                                            new_value
                                        )
                                    )
                            logger.debug(
                                "oid: %s, type_and_value: %s", oid, type_and_value
                            )
                ...
                logger.error("Error in SNMP server loop: %s", ex)
                time.sleep(1)


def main():
    """Main"""
    global config_file, config_last_modified
    ...
    parser.add_argument(
        ...
        dest="port",
        type=int,
        help="port (by default 161 - requires root privileges)",
        default=161,
        required=False,
    )
    parser.add_argument(
        "-c", "--config", type=str, help="OIDs config file", required=False
    )
    ...
    if config_filename:
        try:
            ...
            config_last_modified = os.path.getmtime(config_filename)
        except ConfigError as ex:
            logger.error(ex)
            sys.exit(-1)

    # Start config monitor thread
    config_monitor = threading.Thread(
        target=check_config_updates, args=(oids,), daemon=True
    )
